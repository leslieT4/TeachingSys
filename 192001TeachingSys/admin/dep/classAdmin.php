...
<head>
    ...
    $depName = $_GET["dep"];
    $sql = "select cait_class.stuclass,cait_major.majorname,
            stuyear
    ...
    if ($stmt = $conn->prepare($sql)){
        ...
        $stmt->execute();
        $stmt->bind_result($stuClass,$majorName,$stuYear);
        $stmt->store_result();
    }
    ?>
</head>
...
END;
    ...
    <form method="post" action="classExImport.php" enctype="multipart/form-data">
        <input type="file" name="classfile">
        <input type="submit" value="确定">
        <a href="downClassTemp.php"><input type="button" value="下载模板"></a>
    </form>
	</div>
	<!--当前的位置提示-->
	<div class="side">
